...
class Code(object):
    ...
    def for_a(self, addr):
        return '0' + self.to_bin(addr).zfill(15)

    def for_c(self, dest, comp, jump):
        return '111' + self.comp(comp) + self.dest(dest) + self.jump(jump)

    dest_codes = ['', 'M', 'D', 'MD', 'A', 'AM', 'AD', 'AMD']

    def dest(self, d):
        return self.to_bin(self.dest_codes.index(d)).zfill(3)
    ...
    def jump(self, j):
        return self.to_bin(self.jump_codes.index(j)).zfill(3)
